...
class LitViewController: UIViewController {
    ...
    let request = NSFetchRequest<NSFetchRequestResult>(entityName: "LitQuestion")
    ...
    weak var delegate: DataEnteredDelegate3? = nil
        ...
        else
        {
            ...
            do
            {
                ...
                if numberOfRecord > 0
                {
                    ...
                    self.delegate?.userDidEnterInformation3(username: recordName, gamemode: recordGameMode, points: recordPoints, score: recordScore, totalPoints: String(total), date: recordDate)
                    self.navigationController?.popViewController(animated: true)
                }
